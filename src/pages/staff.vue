...
const fetchData = (currPage) => {
  ...
  // 搜索
  if (data.searchVal) {
    resultData = data.allList.filter((item) => item.name.indexOf(data.searchVal) !== -1)
  } else {
    resultData = [...data.allList]
  }
...
const handleAddSuccess = (info) => {
  data.allList = [{ id: +new Date(), ...info }, ...data.allList]
  fetchData(1)
}
const handleView = (record) => {
  viewModalRef.value.init(record)
}
</script>
...
<template>
  <div class="staff-page">
    ...
    <AddModal ref="addModalRef" @success="handleAddSuccess" />
